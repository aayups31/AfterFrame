...
The starter UI remains a non-binding mock. Migrations 001–014 and the authenticated Postgres worker lifecycle are deployed. The real Supabase/TMDB path completes resolver-verified `IDENTITY` and deterministic `SCOPING`; a deterministic provider transport proves resumable `DISCOVERY`; and the real RESOLUTION executor proves bounded candidate work, atomic source/locator acceptance, same-attempt handoff and takeover, recovered-work skipping, exact stage completion, and `NORMALIZATION` unlock against deployed Postgres. RESOLUTION has a production-composable, disabled-by-default public metadata probe with DNS/IP pinning, redirect-by-redirect revalidation, TLS and port policy, one abortable deadline, bounded headers, and strict body exclusion. The first NORMALIZATION-side capability adds separately disabled, rights-gated hostile-source retrieval with bounded streaming, the same DNS/IP/redirect defenses, HTTPS downgrade and compression rejection, MIME/signature verification, and transient-only handling for link-only bytes. Its deployed lease-fenced acceptance ledger now binds every decision to the original discovery candidate, committed source resolution, active normalization manifest, rights policy, immutable receipt, and snapshot identity; exact retries replay, stale leases cannot write, and link-only bodies remain transient. No retrieved body is durably stored, parsed, or promoted to evidence yet. The next gate is hostile-content parsing, injection screening, and provenance-preserving normalized document trees before exact medium-specific locator validation. The current environment does not claim MAM and no paid live call is enabled. The public production research route remains disabled until exact locator validation, evidence and claim persistence, corroboration, sequencing, provenance, and domain eval gates pass.
