...
The existing starter UI is a mocked vertical slice, not a production research engine. The current durable production work begins in `docs/48-build-checkpoint-04b.md`; the latest completed slice is `docs/60-build-checkpoint-04c-resumable-resolution-executor.md`. Its boundaries, not the mock UI types or routes, are the implementation starting point. Migrations 001–013 are deployed. The next gate is the production public-network metadata transport, followed by hostile-input retrieval, normalization, and exact-locator verification; paid live discovery and the public route remain disabled.
