...
const enabled = process.env.AFTERFRAME_DB_MIGRATION_013_PREFLIGHT === "1";
...
function databaseUrl() {
  ...
  if (value === undefined) {
    throw new Error("Migration 013 preflight requires SUPABASE_DB_URL");
  }
  ...
  if (
    ...
    throw new Error("Migration 013 preflight requires a credentialed PostgreSQL URL");
  }
  return value;
}

describeDatabase("migration 013 production rollback preflight", () => {
  it(
    "applies atomically over 001-012 with a default-deny resolution boundary",
    async () => {
      ...
      try {
        ...
        const relation = await client.query<{
          ...
          `select relation.relrowsecurity as row_security,
           ...
           where namespace.nspname = 'public'
             and relation.relname = 'af_source_resolution_records'`,
        );
        ...
        expect(rpcPrivileges.rows).toHaveLength(3);
        for (const privilege of rpcPrivileges.rows) {
        ...
        const absent = await client.query<{ relation: string | null }>(
          "select to_regclass('public.af_source_resolution_records')::text as relation",
        );
