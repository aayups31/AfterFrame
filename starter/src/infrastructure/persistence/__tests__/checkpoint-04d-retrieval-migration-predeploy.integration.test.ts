...
const enabled = process.env.AFTERFRAME_DB_MIGRATION_014_PREFLIGHT === "1";
...
function databaseUrl() {
  ...
  if (value === undefined) {
    throw new Error("Migration 014 preflight requires SUPABASE_DB_URL");
  }
  ...
  if (
    ...
    throw new Error("Migration 014 preflight requires a credentialed PostgreSQL URL");
  }
  return value;
}

describeDatabase("migration 014 production rollback preflight", () => {
  it(
    "applies atomically over 001-013 with a default-deny retrieval boundary",
    async () => {
      ...
      try {
        ...
        const absent = await client.query<{ relation: string | null }>(
          "select to_regclass('public.af_source_retrieval_records')::text as relation",
        );
